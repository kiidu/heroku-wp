 ...
 */
function virtue_kadence_blocks_notice() {
	if ( ! class_exists( 'virtue_toolkit_welcome' ) || get_transient( 'virtue_theme_blocks_plugin_notice' ) || ! current_user_can( 'manage_options' ) || ! function_exists( 'register_block_type' ) ) {
		return;
	}
	$installed_plugins = get_plugins();
	if ( ! isset( $installed_plugins['kadence-blocks/kadence-blocks.php'] ) ) {
		$button_label = esc_html__( 'Install Kadence Blocks', 'virtue' );
		$data_action  = 'install';
	} elseif ( ! Virtue_Plugin_Check::active_check( 'kadence-blocks/kadence-blocks.php' ) ) {
		$button_label = esc_html__( 'Activate Kadence Blocks', 'virtue' );
		$data_action  = 'activate';
	} else {
		return;
	}
	...
	<div id="message" class="updated kt-plugin-install-notice-wrapper">
		<h3 class="kt-notice-title"><?php echo esc_html__( 'Thanks for choosing the Virtue Theme', 'virtue' ); ?></h3>
		<p class="kt-notice-description"><?php /* translators: %s: <strong> <a> */ printf( esc_html__( 'We have a %1$snew plugin%2$s to extend and enhance Gutenberg for your site. To take full advantage of the Virtue Theme please install the %3$sKadence Blocks%4$s, this adds extra Gutenberg settings and features.', 'virtue' ), '<a href="https://wordpress.org/plugins/kadence-blocks/" target="_blank">', '</a>', '<strong>', '</strong>' ); ?></p>
		<p class="submit">
			...
			<a href="<?php echo esc_url( wp_nonce_url( add_query_arg( 'virtue-kadence-blocks-plugin-notice', 'install' ), 'virtue_blocks_hide_notices_nonce', '_notice_nonce' ) ); ?>" class="notice-dismiss kt-close-guten-notice"><span class="screen-reader-text"><?php esc_html_e( 'Skip', 'virtue' ); ?></span></a>
		</p>
	</div>
	<?php
	wp_enqueue_script( 'kadence-toolkit-install' );
}
add_action( 'admin_notices', 'virtue_kadence_blocks_notice' );

/**
 * Hide Notice
 */
function virtue_hide_blocks_plugin_notice() {
	if ( isset( $_GET['virtue-kadence-blocks-plugin-notice'] ) && isset( $_GET['_notice_nonce'] ) ) {
		if ( ! wp_verify_nonce( wp_unslash( sanitize_key( $_GET['_notice_nonce'] ) ), 'virtue_blocks_hide_notices_nonce' ) ) {
			wp_die( esc_html__( 'Authorization failed. Please refresh the page and try again.', 'virtue' ) );
		}
		set_transient( 'virtue_theme_blocks_plugin_notice', 1, 4 * YEAR_IN_SECONDS );
	}
}
add_action( 'wp_loaded', 'virtue_hide_blocks_plugin_notice' );
